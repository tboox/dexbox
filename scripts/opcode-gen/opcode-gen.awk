...
emission == "dexbox-maximum-values" {
    emissionHandled = 1;

    printf("#define DX_OPCODE_VALUE_MAX     (0x%x)\n", MAX_OPCODE);
    printf("#define DX_OPCODE_PACKED_MAXN   (0x%x)\n", MAX_PACKED_OPCODE + 1);
}

emission == "dexbox-opcode-enum" {
    emissionHandled = 1;

    for (i = 0; i <= MAX_PACKED_OPCODE; i++)
    {
        printf("    DX_OPCODE_%-28s = 0x%02x,\n", packedConstName[i], i);
    }
}

emission == "dexbox-goto-table" {
    emissionHandled = 1;
...
emission == "dexbox-formats" {
    emissionHandled = 1;

    col = 1;
    for (i = 0; i <= MAX_PACKED_OPCODE; i++) {
        value = sprintf("DX_INSTR_FMT_%s,", packedFormat[i]);
        col = colPrint(value, (i == MAX_PACKED_OPCODE), col, 7, 9, "    ");
    }
}

emission == "dexbox-index-types" {
    emissionHandled = 1;
...
function initIndexTypes() {
    indexTypeValues["unknown"]              = "DX_INSTR_INDEX_TYPE_UNKNOWN";
    indexTypeValues["none"]                 = "DX_INSTR_INDEX_TYPE_NONE";
    indexTypeValues["varies"]               = "DX_INSTR_INDEX_TYPE_VARIES";
    indexTypeValues["type-ref"]             = "DX_INSTR_INDEX_TYPE_TYPE_REF";
    indexTypeValues["string-ref"]           = "DX_INSTR_INDEX_TYPE_STRING_REF";
    indexTypeValues["method-ref"]           = "DX_INSTR_INDEX_TYPE_METHOD_REF";
    indexTypeValues["field-ref"]            = "DX_INSTR_INDEX_TYPE_FIELD_REF";
    indexTypeValues["inline-method"]        = "DX_INSTR_INDEX_TYPE_INLINE_METHOD";
    indexTypeValues["vtable-offset"]        = "DX_INSTR_INDEX_TYPE_VTABLE_OFFSET";
    indexTypeValues["field-offset"]         = "DX_INSTR_INDEX_TYPE_FIELD_OFFSET";
    indexTypeValues["method-and-proto-ref"] = "DX_INSTR_INDEX_TYPE_METHOD_AND_PROTO_REF";
    indexTypeValues["call-site-ref"]        = "DX_INSTR_INDEX_TYPE_CALL_SITE_REF";
    indexTypeValues["method-handle-ref"]    = "DX_INSTR_INDEX_TYPE_METHOD_HANDLE_REF";
    indexTypeValues["proto-ref"]            = "DX_INSTR_INDEX_TYPE_PROTO_REF";
}

# Initialize the flags data.
function initFlags() {
    flagValues["branch"]        = "DX_INSTR_FLAGS_CAN_BRANCH";
    flagValues["continue"]      = "DX_INSTR_FLAGS_CAN_CONTINUE";
    flagValues["switch"]        = "DX_INSTR_FLAGS_CAN_SWITCH";
    flagValues["throw"]         = "DX_INSTR_FLAGS_CAN_THROW";
    flagValues["return"]        = "DX_INSTR_FLAGS_CAN_RETURN";
    flagValues["invoke"]        = "DX_INSTR_FLAGS_INVOKE";
